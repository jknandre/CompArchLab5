...
int v_sr_ld_cc, v_agex_ld_reg, v_mem_ld_reg, v_mem_ld_cc, v_agex_ld_cc;
int mempcmux;
...
int TRAP_PC, TARGET_PC, MEM_STALL;
int cycleCounter = 0;
int V_DCACHE_EN = 0;
int ldde;
/************************* MEM_stage() *************************/
void MEM_stage() {
  ...
  int read_word, dcache_r, write_word, mem_w0 = 0, mem_w1 = 0;
  if(cycleCounter < 3){
    PS.SR_V = 0;//ready = 0;
  }
  if(Get_DCACHE_EN(PS.MEM_CS)){
    if(cycleCounter == 4){
      ...
      V_DCACHE_EN = DCACHE_EN && PS.MEM_V;
...
void AGEX_stage() {
  ...
  LD_MEM != mem_stall; // need to implement mem_stall
  ...
  if (LD_MEM) {
    ...
    if(Get_ADDR1MUX(PS.AGEX_CS) == 0){
      ...
      value2 = (Sext((PS.AGEX_IR & 63),6) << 1);
    }else if(Get_ADDR1MUX(PS.AGEX_CS) == 2){
      value2 = (Sext((PS.AGEX_IR & 63),9) << 1);
    }else if(Get_ADDR1MUX(PS.AGEX_CS) == 3){
      value2 = (Sext((PS.AGEX_IR & 63),11) << 1);
    }
    NEW_PS.MEM_ADDRESS = value1 + value2;
  }else{
    NEW_PS.MEM_ADDRESS = ((PS.AGEX_IR & 255) < 1);
  }
  if(Get_ALU_RESULTMUX(PS.AGEX_CS) == 0){
    int shiftAmount = PS.AGEX_IR & 0xF;
    int leftright = (PS.AGEX_IR & 0x0010) >> 4;

    if(leftright == 0){
      NEW_PS.MEM_ALU_RESULT = PS.AGEX_SR1 << shiftAmount;
    }
    else{
      if(PS.AGEX_IR & 0x0020){
        int value = PS.AGEX_SR1 >> shiftAmount;
        if(PS.AGEX_SR1 & 0x8000){
          NEW_PS.MEM_ALU_RESULT = Sext(value, 16-shiftAmount);
        }
      }else{
        NEW_PS.MEM_ALU_RESULT = PS.AGEX_SR1 >> shiftAmount;
      }
    ...
    int valuea = PS.AGEX_SR1;
    int valueb;
    if(Get_SR2MUX == 0){
      valueb = PS.AGEX_SR2;
    }else{
      valueb = Sext(PS.AGEX_IR, 5);
    }
    if(Get_ALUK == 0){ // add
      NEW_PS.MEM_ALU_RESULT = valuea + valueb;
    }
    else if(Get_ALUK == 1){ // and
      NEW_PS.MEM_ALU_RESULT = valuea & valueb;
    }
    else if(Get_ALUK == 2){ //xor
      NEW_PS.MEM_ALU_RESULT = valuea ^ valueb;
      
    } else{ //passb
      NEW_PS.MEM_ALU_RESULT = Low16bits(valueb); 
    }
  }
...
void DE_stage() {
  ...
  int controlstorevalue = (PS.DE_IR & 0xF000 >> 10) + (PS.DE_IR & 0x0020 >> 5);
  int read_word;
  icache_access(controlstorevalue, &read_word, &icache_r);
  //NEW_PS.AGEX_CS = 
  if(v_sr_ld_reg || v_mem_ld_reg || v_agex_ld_reg){
    dep_stall = 1;
  }
  if(v_sr_ld_cc || v_mem_ld_cc || v_agex_ld_cc){
    dep_stall;
  }

  NEW_PS.AGEX_SR1 = REGS[(PS.DE_IR & 0x01C0)];
  if(PS.DE_IR & 0x0400){
    NEW_PS.AGEX_SR2 = REGS[(PS.DE_IR & 0x01C0)>>9];
  }else{
    NEW_PS.AGEX_SR2 = REGS[(PS.DE_IR & 0x0005)];
  }
  NEW_PS.AGEX_CC = (N<<2)+(Z<<1)+(P);
  if( v_sr_ld_cc == 1){ // implement v_sr_ld_cc
    
  }
  if(Get_DRMUX(controlstorevalue)) {// implement
    NEW_PS.AGEX_DRID = 7;
  }else{
    NEW_PS.AGEX_DRID = ((PS.DE_IR & 0x01C0)>>9);
  } 
  if(PS.DE_V){
      v_de_br_stall = CONTROL_STORE[controlstorevalue][BR_STALL];
  }
